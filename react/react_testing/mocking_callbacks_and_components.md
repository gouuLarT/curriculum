...
Two tests and we are done with this component. You should be already familiar how the first test works. Take some time to figure out what functions come from which package.

For the second test we mock the `onClick` handler using one of Vitest's functions, `vi.fn()`. Then we assert that it is called when the button is clicked. 
